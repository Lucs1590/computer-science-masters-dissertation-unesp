...
No âmbito das arquiteturas voltadas para segmentação semântica, a evolução trouxe a U-Net \citep{Ronneberger2015U-net:Segmentation}, que se destacou após o período das FCNs. Introduzida em cerca de 2015, a U-Net rapidamente ganhou notoriedade, superando a popularidade das mencionadas FCNs \citep{Sultana2020EvolutionSurvey}.

As U-Nets operam com um modelo de codificação (\textit{encoder}) que progressivamente reduz a resolução espacial dos dados, seguido por um modelo de decodificação (\textit{decoder}) que restaura a resolução original e realiza a segmentação. A estrutura se assemelha a um "U", como retrata a Figura \ref{semantic:fig:unet}, o que levou à sugestão do nome da arquitetura. A conexão entre as camadas de \textit{encoder} e \textit{decoder}, conhecida como \quotes{skip connection}, permite a preservação de informações de baixo nível e ajuda a mitigar problemas de perda de detalhes durante a redução da resolução.
...
As vantagens inerentes a essa arquitetura incluem uma capacidade robusta para lidar com imagens de tamanho variado, a habilidade de aprender representações significativas em diferentes níveis de abstração e a redução da tendência do modelo de gerar segmentações excessivamente granulares. No entanto, é importante observar que, como qualquer abordagem, a U-Net também possui desvantagens. Por exemplo, ela pode enfrentar dificuldades ao segmentar objetos de tamanhos muito pequenos ou ao tratar de casos em que a forma dos objetos varia consideravelmente.

Voltando ao contexto mais amplo, onde a segmentação semântica tem como objetivo atribuir rótulos de classe a cada pixel em uma imagem, a arquitetura da U-Net se mostra relevante e eficaz, sendo exemplificado pelos resultados obtidos por X. No entanto, questões que exploram a preservação da espacialidade da imagem durante a segmentação nesse tipo de estrutura ainda são pouco explorados, principalmente quando a proposta está em variações das tradicionais camadas de \textit{pooling}.
