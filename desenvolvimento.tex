...
É importante ressaltar que para todas as arquiteturas, 

Na arquitetura VGG-16, que foi aplicada na tarefa de classificação de imagens nos conjuntos de dados CIFAR-100 e \textit{Food}-101, as camadas de \textit{Max Pooling} e \textit{Avg Pooling} foram substituídas pela BPCAPooling. Tal modificação visa melhorar a capacidade do modelo de capturar características discriminativas mais relevantes, proporcionando assim uma melhoria potencial no desempenho do modelo.


Da mesma forma, na arquitetura U-Net-Like, empregada para segmentação semântica no conjunto de dados Oxford-IIIT Pets, as camadas de \textit{Max Pooling} foram igualmente trocadas pela BPCAPooling. Este rearranjo procura potenciar o desempenho do modelo no que diz respeito à detecção de bordas e regiões de interesse, incrementando assim a precisão da segmentação semântica produzida.


Por fim, na arquitetura U-Net, também utilizada para a tarefa de segmentação, procedeu-se à mesma substituição das camadas de \textit{Max Pooling} pelas camadas de BPCAPooling. Esta modificação visa aprimorar os recursos de segmentação da arquitetura U-Net, promovendo assim resultados mais precisos e robustos.

Ao longo da dissertação, será discutido o impacto desta alteração na performance dos modelos e sua relevância no contexto da aprendizagem profunda de maneira mais aprofundada.
