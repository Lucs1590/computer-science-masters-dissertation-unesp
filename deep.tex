...
O \textit{Adaptive Linear Neuron} (ADALINE), proposto por Widrow e Hoff \citep{Widrow199030YO}, é um marco histórico na evolução dos algoritmos de aprendizado de máquina. Ele representa uma significativa progressão em relação ao Perceptron, pois emprega uma função linear para calcular sua saída a partir das entradas. Enquanto o Perceptron atualiza seus pesos com base em erros de classificação binários, o ADALINE ajusta os pesos por meio de uma função de erro contínuo, utilizando a diferença entre a saída esperada e a saída calculada pela função linear. Essa abordagem mais refinada permite uma correção mais precisa dos pesos, demonstrando uma evolução significativa na capacidade de aprendizado dos modelos.
...
Embora o ADALINE seja restrito a problemas lineares, seu desenvolvimento foi um passo crucial para os modelos subsequentes, que generalizam a abordagem para contextos não lineares, aprimorando as capacidades de modelagem e de aprendizado das redes.
