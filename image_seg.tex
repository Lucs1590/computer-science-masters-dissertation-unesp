...
\resizebox{\textwidth}{!}{%
    ...
    \hline
    Segmentação Baseada em Agrupamentos & \begin{tabular}[c]{@{}l@{}}Divide os \textit{pixels} da imagem\\em k \textit{clusters} homogêneos e\\exclusivos obtendo, assim,\\as segmentações.\end{tabular}             & \begin{tabular}[c]{@{}l@{}}- Conceitos já estabelecidos;\\ - Recomendado para aplicações em tempo\\~ real;\\ - Bom desempenho para pequenos\\~\textit{datasets};\\ - Não precisa de tempo de treinamento.\end{tabular} & \begin{tabular}[c]{@{}l@{}}- Pode demandar muito tempo para\\~encontrar a minimização da função\\~de custo;\\ - Não é adequado para agrupar \\\textit{~clusters} não convexos.\end{tabular}         \\ 
    \hline
    Segmentação Baseada em Grafos       & \begin{tabular}[c]{@{}l@{}}Representa uma imagem como\\um grafo, onde cada nó\\é um \textit{pixel} e as arestas\\são a relação entre os \textit{pixels}\\vizinhos.\end{tabular}     & \begin{tabular}[c]{@{}l@{}}- Robusto e preciso;\\ - Flexível com variados tipos de imagens;\\ - Compatibilidade com outros métodos\\~ de segmentação.\end{tabular}                                                     & \begin{tabular}[c]{@{}l@{}}- Parâmetros podem ser difíceis de determinar;\\ - Sensível aos parâmetros;\\ - Podem ocorrer segmentações em excesso;\\~especialmente em imagens ruidosas.\end{tabular} \\
    \hline
    Segmentação  \textit{Superpixel}    & \begin{tabular}[c]{@{}l@{}}Agrupa \textit{pixels} que compartilham\\características comuns para\\formar uma maior unidade\\coesa denominada \textit{superpixel}.\end{tabular}       & \begin{tabular}[c]{@{}l@{}}- Reduz a complexidade dos dados;\\ - Adere bem às bordas do objeto;\\ - Maior velocidade de processamento.\end{tabular}                                                                    & \begin{tabular}[c]{@{}l@{}}- Pode perder detalhes finos das imagens;\\ - Parâmetros podem ser difíceis de determinar.\end{tabular}                                                                  \\
    \hline
