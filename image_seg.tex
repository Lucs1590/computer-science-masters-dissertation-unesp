...
De um modo geral, a segmentação por meio da determinação de um limiar acontece em imagens que utilizam a escala de cinza ou algum sistema de cor que possua um canal voltado para intensidade, como o sistema HSV \citep{schneider2003experimentos}, visto que quando no sistema correto, há a determinação de um valor entre o fundo e a área de interesse da imagem. Dessa forma, os pixels acima do valor médio determinado são categorizados como área de intensidade e os inferiores são categorizados como fundo \citep{pedrini2008analise}, como é possível observar na Equação \ref{segment:eq:1}, em que $T$ representa o limiar determinado, $f(x,y)$ é um pixel da imagem $f$ e $g(x,y)$ é este pixel limiarizado:
...
Esta técnica é ilustrada na Figura \ref{segment:fig:4}, onde a Figura \ref{segment:fig:4.1} mostra a imagem original com os pixels sementes em vermelho ($f(r,s)$). As Figuras \ref{segment:fig:4.2} e \ref{segment:fig:4.3} representam, respectivamente, o crescimento da região com limiares $T = 3$ e $T = 6$.
...
Ainda sobre o operador laplaciano, pode se dizer que há uma necessidade de que o coeficiente do pixel central resulte em um valor positivo e que os seus \textit{pixels} externos sejam negativos \citep{pedrini2008analise}. Assim, considerando uma matriz de vizinhança-8, pode-se exemplificar uma implementação do operador Laplaciano que considera as derivadas segundas em linhas e colunas tendo por base a matriz $H_1$ representada na Equação \ref{segment:eq:7}, resultando em aplicações como as visualizadas na Figura \ref{segment:fig:8}, sendo: 
...
\begin{enumerate}
    ...
    \item Marcação dos pixels vizinhos aos marcadores como pertencentes às suas respectivas bacias de retenção;
...
\resizebox{\textwidth}{!}{%
    ...
    \hline
    Segmentação Baseada em Agrupamentos & \begin{tabular}[c]{@{}l@{}}Divide os pixels da imagem\\em k \textit{clusters} homogêneos e\\exclusivos obtendo, assim,\\as segmentações.\end{tabular}             & \begin{tabular}[c]{@{}l@{}}- Conceitos já estabelecidos;\\ - Recomendado para aplicações em tempo\\~ real;\\ - Bom desempenho para pequenos\\~\textit{datasets};\\ - Não precisa de tempo de treinamento.\end{tabular} & \begin{tabular}[c]{@{}l@{}}- Pode demandar muito tempo para\\~encontrar a minimização da função\\~de custo;\\ - Não é adequado para agrupar \\\textit{~clusters} não convexos.\end{tabular}         \\ 
    \hline
    Segmentação Baseada em Grafos       & \begin{tabular}[c]{@{}l@{}}Representa uma imagem como\\um grafo, onde cada nó\\é um pixel e as arestas\\são a relação entre os pixels\\vizinhos.\end{tabular}     & \begin{tabular}[c]{@{}l@{}}- Robusto e preciso;\\ - Flexível com variados tipos de imagens;\\ - Compatibilidade com outros métodos\\~ de segmentação.\end{tabular}                                                     & \begin{tabular}[c]{@{}l@{}}- Parâmetros podem ser difíceis de determinar;\\ - Sensível aos parâmetros;\\ - Podem ocorrer segmentações em excesso;\\~especialmente em imagens ruidosas.\end{tabular} \\
    \hline
    Segmentação  \textit{Superpixel}    & \begin{tabular}[c]{@{}l@{}}Agrupa pixels que compartilham\\características comuns para\\formar uma maior unidade\\coesa denominada superpixel.\end{tabular}       & \begin{tabular}[c]{@{}l@{}}- Reduz a complexidade dos dados;\\ - Adere bem às bordas do objeto;\\ - Maior velocidade de processamento.\end{tabular}                                                                    & \begin{tabular}[c]{@{}l@{}}- Pode perder detalhes finos das imagens;\\ - Parâmetros podem ser difíceis de determinar.\end{tabular}                                                                  \\
    \hline
    Segmentação \textit{Watershed}      & \begin{tabular}[c]{@{}l@{}}Trata a imagem como um\\relevo topográfico, inundando\\todas as depressões a partir\\dos mínimos locais.\end{tabular}                  & \begin{tabular}[c]{@{}l@{}}- Recomendável para imagens com \\~bordas claras e bem definidas;\\ - Baixo custo computacional.\end{tabular}                                                                               & \begin{tabular}[c]{@{}l@{}}- Sensível a ruídos;\\ - Pode produzir segmentos excessivos.\end{tabular}                                                                                                \\
    \hline
    Segmentação \textit{Livewire}       & \begin{tabular}[c]{@{}l@{}}Abordagem interativa que elege um\\caminho de menor custo entre\\dois pontos escolhidos pelo\\usuário.\end{tabular}                    & \begin{tabular}[c]{@{}l@{}}- Proporciona resultados de segmentação\\~precisos;\\ - Aproveita a intuição do usuário;\\ - Eficiência em imagens médicas com\\~ bordas claras e consistentes.\end{tabular}                 & \begin{tabular}[c]{@{}l@{}}- Desempenho insatisfatório em imagens\\~com ruído, baixo contraste ou\\~descontinuidade nas bordas.\end{tabular}                                                        \\
    \hline
