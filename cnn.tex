...
Neste capítulo, é evidenciada a importância das CNNs na abordagem de problemas espaciais, especialmente no domínio de imagens. São abordados detalhes sobre como as características são extraídas por meio de filtros e camadas convolucionais (Seção \ref{cnn:conv}), além da forma como as técnicas de \textit{pooling} são empregadas para reduzir a dimensionalidade dos dados gerados pelas convoluções. Também se destaca a relevância das camadas de saída na definição das classes presentes na imagem, assim como a aplicação de \textit{dropout} para mitigar problemas de \textit{overfitting}.
